...
<div class="page-header ">
    <h2 class="col-sm-offset-4 text-primary">修改新闻</h2>
</div>
<div class="container">
    <s:form method="post" action="newsUpdated" class="form-horizontal">
        <s:hidden name="news.id" value="%{news.id}"/>
        <s:hidden name="news.clicks" value="%{news.clicks}"/>
        <s:hidden name="news.issueTime" value="%{news.issueTime}"/>
        <s:hidden name="news.imgs" value="%{news.imgs}"/>
        <div class="form-group">
            <label  class="col-sm-2 control-label">标题</label>
            <div class="col-sm-3">
                <%--<s:textfield name="news.title" value="%{news.title}"/>--%>
                <input type="text" id="inputUsername" name="news.title" class="form-control" placeholder="请输入标题"
                       autofocus
                       required value="<s:property value="news.title"/>"/>
            </div>
        </div>


        <div class="form-group">
            <label for="inputSex" class="col-sm-2 control-label">类型</label>
            <div class="col-sm-3">
                ...
                <%--</select>--%>
                    <s:select style="margin-top:-220%;margin-left:130%;" id="inputSex" name="cid" list="categorys" class="form-control" value="%{news.category.id}" listKey="id" listValue="categoryName" headerKey="-1" headerValue="---请选择类型---"></s:select>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-6">
                <textarea id="inputIntro" rows="8" cols="90" class="form-control" name="news.content"><s:property value="news.content"/> </textarea>
                <%--<textarea id="inputIntro" name="intro" class="form-control" cols="100" rows="10" required style="overflow-y:scroll"></textarea>--%>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
                <%--<s:textfield id="inputEmail" name="news.froms"/>--%>
                <input id="inputEmail" type="text" class="form-control" name="news.froms" placeholder="请输入来源" required value="<s:property value="news.froms"/>">
            </div>
        </div>

        <div class="row">
            <div class="col-sm-2 col-sm-offset-2">
                <button type="submit" class="btn btn-success btn-lg btn-block glyphicon glyphicon-ok">&nbsp;更新</button>
            </div>
            <div class="col-sm-2">
